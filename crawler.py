...
def parse_weather_forecast(data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        def safe_extract_value(element_name, period_time):
            ...
            for t in element_map.get(element_name, []):
                ...
                if t.get('startTime') == period_time.get('startTime'):
                    # 這是關鍵修正：檢查 'parameter' 鍵是否存在
                    parameter = t.get('parameter')
                    if parameter:
                        # 對 Wx, CI 等使用 parameterName (描述性文字)
                        if element_name in ['Wx', 'CI']:
                            return parameter.get('parameterName', 'N/A')
                        # 對 PoP, MinT, MaxT 等使用 parameterName (數值)
                        elif element_name in ['PoP', 'MinT', 'MaxT']:
                            return parameter.get('parameterName', 'N/A')
            return 'N/A'
        ...
        for period in wx_times:
            ...
            # 提取天氣現象 (Wx) - 直接從 Wx 的 period 中提取，使用 'parameter' 鍵
            weather_value = period.get('parameter', {}).get('parameterName', 'N/A')
            
            # 提取 PoP, MinT, MaxT
            # 使用輔助函式，並傳入 element_name 區分提取邏輯 (雖然在這個資料集都一樣是 parameterName)
            ...
            forecasts.append({
                ...
                'End Time': end_time,
                'Weather': weather_value, # 使用直接提取的 Wx 描述
                'PoP (%)': pop_value,
                'Min Temp (°C)': min_t,
                'Max Temp (°C)': max_t,
            })
        ...
        st.error(f"資料解析發生最終錯誤: {e}，請檢查 JSON 結構是否再次變動。")
        return []
